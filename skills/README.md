...
| [**Bib Update**](bib-update/) | `/bib-update` | Assembles or refreshes `references/references.bib` from per-paper metadata blocks written by `/split-pdf`, `/read-pdf`, or `/wiki-update`. Uses DOI, CrossRef, OpenAlex, then approved fallback entries, and skips keys already present in default mode. Companion to `/bibcheck`, which audits existing `.bib` files. [See documentation →](bib-update/) |
